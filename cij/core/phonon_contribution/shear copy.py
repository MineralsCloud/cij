...
class ShearElasticModulusPhononContribution:
    ...
    def fictitious_strain_rotated(self):
        ...
        '''
        print("Fictitious rotated -> ", self.key, numpy.diag(numpy.linalg.eig(self.fictitious_strain)[0]))
    ...
    def transformation_matrix(self):
        ...
        '''
        print("Transformation matrix -> ", self.key, numpy.linalg.eig(self.fictitious_strain)[1])
    ...
    def get_elastic_modulus(self, key: C_) -> numpy.ndarray:
        ...
        # if self.key == c_(15):
        #     i,j = key.v
        #     with open(f"dc{i}{j}.txt", "w") as fp:
        #         import pandas
        #         df = pandas.DataFrame(self.modulus[key])
        #         df.index = self.calculator.t_array
        #         df.columns = self.calculator.v_array
    ...
    def get_elastic_modulus_rotated(self, key: C_) -> numpy.ndarray:
        ...
        # if self.key == c_(15):
        #     i,j = key.v
        #     with open(f"dc{i}{j}p.txt", "w") as fp:
        #         import pandas
        #         df = pandas.DataFrame(self.modulus_rotated[key])
        #         df.index = self.calculator.t_array
        #         df.columns = self.calculator.v_array
        #         fp.write(df.to_string())
