...
#     / scipy.constants.physical_constants["Avogadro constant"][0] / numpy.pi / 2
_h = scipy.constants.physical_constants["Planck constant in eV/Hz"][0] * scipy.constants.physical_constants["speed of light in vacuum"][0]
...
h = units.Quantity(_h, units.eV / units.Hz * units.m / units.s).to(units.rydberg * units.cm).magnitude
...
class LongitudinalElasticModulusPhononContribution(ElasticModulus):
    ...
    def thermal_contribution(self) -> numpy.ndarray:
        ...
        # print(self, self.prefactors)

        import pandas
        with open("cii-tt.txt", "w") as fp:
            fp.write(pandas.DataFrame(ret).to_string())
    ...
    def isothermal_to_adiabatic(self):
        ...
        import pandas
        with open("CfromS.txt", "w") as fp:
            fp.write(pandas.DataFrame(ret).to_string())
        with open("t.txt", "w") as fp:
            fp.write(pandas.DataFrame(self.t_array).to_string())
        with open("p.txt", "w") as fp:
            fp.write(pandas.DataFrame(self.calculator.qha_calculator.volume_base.pressures).to_string())
...
class OffDiagonalElasticModulusPhononContribution(LongitudinalElasticModulusPhononContribution):
    ...
    def thermal_contribution(self):
        ...
        print("MG0", self.mode_gamma[0])
        print("MG2", self.mode_gamma[2])

        import pandas
        with open("cij-tt.txt", "w") as fp:
            fp.write(pandas.DataFrame(ret).to_string())
        with open("cij-tt-p.txt", "w") as fp:
            fp.write(pandas.DataFrame(ret + (
            + self.qha_calculator.volume_base.pressures
            - self.calculator.static_p_array[nax, :]
        )).to_string())
 
        
        with open("gamma1.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(self.calculator.mode_gamma[1]),
                index=self.calculator.v_array
            ).to_string())
        with open("gamma2.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(self.calculator.mode_gamma[2]),
                index=self.calculator.v_array
            ).to_string())
        with open("Q1.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(self.Q1),
            ).to_string())
        with open("Q2.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(self.Q2),
            ).to_string())
        with open("gamma10t.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(
                    self.Q1 * self.mode_gamma[0][nax,:,:,:]
                ) / self.prefactors[0]
            ).to_string())
        with open("gamma11t.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(
                    self.Q1 * self.mode_gamma[1][0][nax,:,:,:]
                ) / self.prefactors[1][0]
            ).to_string())
        with open("gamma12t.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(
                    self.Q1 * self.mode_gamma[2][nax,:,:,:]
                ) / self.prefactors[2]
            ).to_string())
        with open("gamma21t.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(
                    self.Q2 * self.mode_gamma[1][0][nax,:,:,:]
                ) / self.prefactors[1][0]
            ).to_string())
        with open("gamma22t.txt", "w") as fp:
            fp.write(pandas.DataFrame(
                self.average_over_modes(
                    self.Q2 * self.mode_gamma[2][nax,:,:,:]
                ) / self.prefactors[2]
            ).to_string())
        return ret
