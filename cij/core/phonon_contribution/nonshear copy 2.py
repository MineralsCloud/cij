...
class LongitudinalElasticModulusPhononContribution(ElasticModulus):
    ...
    def thermal_contribution(self) -> numpy.ndarray:
        ...
        with open("gamma0w.txt", "w") as fp:
            df = pandas.DataFrame(
                self.average_over_modes(
                    self.mode_gamma[0] * self.freq_array
                ) / self.prefactors[0]
            )
            df.index = self.v_array
            fp.write(df.to_string())

        with open("gamma1w.txt", "w") as fp:
            df = pandas.DataFrame(
                self.average_over_modes(
                    self.mode_gamma[1][0] * self.freq_array
                ) / self.prefactors[1][0]
        ...
        with open("gamma11t.txt", "w") as fp:
            ...
            df.index = self.t_array
            df.columns = self.v_array
            fp.write(df.to_string())
